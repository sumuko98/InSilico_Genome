...
def parse_cigar(cigar_str: str) -> Tuple[int, int, bool, bool]:
    """
    Parse CIGAR string to extract soft-clip and hard-clip information.
    
    Args:
        cigar_str: CIGAR string from alignment
        
    Returns:
        Tuple of (left_softclip_length, right_softclip_length, has_left_clip, has_right_clip)
        where has_left/right_clip indicates presence of soft or hard clip
    """
    if not cigar_str:
    ...
    # Check first operation for left clip (soft or hard)
    if operations and operations[0][1] in ('S', 'H'):
        has_left_clip = True
        if operations[0][1] == 'S':
            left_clip = operations[0][0]
    
    # Check last operation for right clip (soft or hard)
    if operations and operations[-1][1] in ('S', 'H'):
        has_right_clip = True
        if operations[-1][1] == 'S':
            right_clip = operations[-1][0]
    
    return left_clip, right_clip, has_left_clip, has_right_clip
...
def analyze_reads_at_junctions(
    ...
    Rules (pysam coordinate conventions):
    - reference_start is 0-based leftmost position
    - reference_end is 0-based exclusive position
    
    Read selection criteria:
    1. EXCLUDE reads that span the junction: reference_start < junction < reference_end
    2. INCLUDE reads that stop at junction on the left: reference_end == junction AND has right-end clip
    3. INCLUDE reads that stop at junction on the right: reference_start == junction AND has left-end clip
    ...
    for chrom_id in chroms_to_analyze:
        ...
        try:
            ...
            for read in samfile.fetch(chrom_id):
                ...
                # Get read alignment positions (pysam coordinates: 0-based)
                read_start = read.reference_start  # 0-based leftmost
                read_end = read.reference_end      # 0-based exclusive
                
                if read_end is None:
                    continue
                
                # Parse CIGAR for clips
                cigar_str = read.cigarstring if read.cigarstring else ""
                left_clip, right_clip, has_left_clip, has_right_clip = parse_cigar(cigar_str)
                
                # Check each junction site
